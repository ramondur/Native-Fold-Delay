...
pdb_file_name <- "AF-P23869-F1-model_v6.pdb"
#Set average translation rate 
transrate <- 20
```
...
```{r}
list.of.packages <- c("ggplot2", "bio3d", "circlize", "dplyr", "tidyr", "RColorBrewer", "stringr", "tibble","ggpubr","jsonlite")
...
The next chunk of code calculates the Native Fold Delay (NFD) of a protein. In other words, it calculates the number of residues that need to be synthesized before residue (i) can engage with all its native interaction partners. NFD can also be expressed in time units by factoring the translation rates of the different residues between i and j (here we are using an average translation rate for all residues).

The output of this code is a contact map plot (contact_map_nopae) showing the furthest away C-terminal interactor of every residue and the per-residue NFD profile of our protein (folddelay_profile_nopae and folddelay_table_nopae).
...
This chunk, as the one above, calculates the Native Fold Delay of a protein. However, here interactions are filtered using the Predicted Aligned Error (PAE) metric from AlphaFold. We used this metric to filter out interactions between residues whose relative positions concerning each other are predicted with low confidence since these interactions most probably do not occur in the actual structure. Therefore, this code only works with AlphaFold models.

The output of this code is a contact map plot (contact_map_pae) showing the furthest away C-terminal interactor of every residue and the per-residue NFD profile of our protein (folddelay_profile_pae and folddelay_table_pae). 
...
version_current <- strsplit(output_file, "_")[[1]][2]
url <- paste0("https://alphafold.ebi.ac.uk/files/AF-",uniprot_id_current,"-F1-predicted_aligned_error_",version_current,".json")
download.file(url, paste("tmp/", "pae_file.json", sep = ""), mode = "wb")
json_data <- fromJSON("tmp/pae_file.json")
